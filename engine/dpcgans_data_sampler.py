...
class DataSampler(object):
    ...
    def __init__(self, data, output_info, log_frequency):
            ...
            if is_discrete_column(column_info_primary):
                ...
                for index_secondary in range(index_primary + 1, len(output_info)):
                    ...
                    if is_discrete_column(column_info_secondary):
                        ...
                        rid_by_cat_pair = []
                        combine_pair_data = np.append(
                            data[:, st_primary:ed_primary],
                            data[:, st_secondary:ed_secondary],
                            axis=1,
                        )
                        combine_pair_data_decimal = reduce(
                            lambda a, b: 2 * a + b, combine_pair_data.transpose()
                        )
                        unique_pair, counts_pair = np.unique(
                            combine_pair_data_decimal, return_counts=True
                        )

                        for uni_value in unique_pair:
                            rid_by_cat_pair.append(
                                np.where(combine_pair_data_decimal == uni_value)[0]
                            )
                        ...
                        st_secondary += sum(
                            [span_info.dim for span_info in column_info_secondary]
                        )
        ...
        if (
            len(self._categories_each_column) > 0
        ):  # if self._categories_each_column is not empty
            second_max_category = np.partition(
                self._categories_each_column.flatten(), -2
            )[-2]
        else:
            second_max_category = 0

        self._discrete_pair_cond_st = np.zeros(
            (
                int(((n_discrete_columns) * (n_discrete_columns - 1)) / 2),
                int((max_category + 1) * (second_max_category + 1)),
            ),
            dtype="int32",
        )
        self._discrete_pair_n_category = np.zeros(
            int(((n_discrete_columns) * (n_discrete_columns - 1)) / 2), dtype="int32"
        )
        self._discrete_column_pair_prob = np.zeros(
            (
                int(((n_discrete_columns) * (n_discrete_columns - 1)) / 2),
                int((max_category + 1) * (second_max_category + 1)),
            )
        ...
        for index_primary in range(
            0, len(output_info)
        ):  # [2, 3, 3, 2] (2,3),(2,3),(2,2),,,(3,3),(3,2),,,,,(3,2)
            column_info_primary = output_info[index_primary]
            if is_discrete_column(column_info_primary):
                span_info_primary = column_info_primary[0]
                ed_primary = st_primary + span_info_primary.dim
                ...
                for index_secondary in range(index_primary + 1, len(output_info)):
                    ...
                    if is_discrete_column(column_info_secondary):
                        ...
                        unique_pair, counts_pair = np.unique(
                            ...
                            return_counts=True,
                        )  # counts_pair --> category_freq

                        pair_prob = counts_pair / np.sum(
                            counts_pair
                        )  ###???? Order in the array??
                        self._discrete_column_pair_prob[
                            current_id_pair, : len(unique_pair)
                        ] = pair_prob

                        self._discrete_pair_cond_st[
                            current_id_pair, 0 : len(unique_pair)
                        ] = unique_pair  ## current_cond_st_pair
                        self._discrete_pair_n_category[current_id_pair] = len(
                            unique_pair
                        )
                        current_cond_st_pair += (
                            span_info_primary.dim * span_info_secondary.dim
                        )
    ...
    def _random_choice_prob_pairs(self, converted_paired_discrete_column_id):
        pair_probs = self._discrete_column_pair_prob[
    ...
    def sample_condvec_pair(self, batch):
        ...
        for iter_gen in range(0, batch):
            paired_discrete_column_id.append(
                np.random.choice(np.arange(self._n_discrete_columns), 2, replace=False)
            )

        # convert paired_discrete_column_id to current_id_pair type
        converted_paired_discrete_column_id = []
        for each in paired_discrete_column_id:
            converted_paired_discrete_column_id.append(
                self.pair_id_dict[tuple(self.get_position[np.sort(each)])]
            )
        ...
        pair_id_in_col = self._random_choice_prob_pairs(
            converted_paired_discrete_column_id
        )  # category_id_in_col: (0, max_num_cateogies), --> [0,2,1,0,2,1...]
        pair_id_decimal = (
            (
                self._discrete_pair_cond_st[
                    np.expand_dims(converted_paired_discrete_column_id, axis=1),
                    np.expand_dims(pair_id_in_col, axis=1),
                ]
            )
            .astype(int)
            .flatten()
        ...
        for itr_decimal in range(0, len(pair_id_decimal)):
            ...
            pair_id_binary = list(
                np.binary_repr(
                    pair_id_decimal[itr_decimal], width=(pair_categories.sum())
                )
            )
            ...
            pair_primary_secondary_cat.append(
                [pair_primary_position, pair_secondary_position]
            )

            pair_primary_secondary_col.append(
                self._discrete_column_cond_st[np.where(mask_pair[itr_decimal] == 1)[0]]
            )

        pair_id_all_positions = np.add(
            np.array(pair_primary_secondary_col), np.array(pair_primary_secondary_cat)
        )
        ...
        return (
            cond_pair,
            mask_pair,
            np.array(converted_paired_discrete_column_id),
            pair_id_in_col,
        )
