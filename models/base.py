...
class BaseSynthesizer:
    ...
    def _update_default_transformers(self):
        if self._model_sdtype_transformers is not None:
            for sdtype, transformer in self._model_sdtype_transformers.items():
                self._data_processor._update_transformers_by_sdtypes(
                    sdtype, transformer
                )

    def __init__(
        self, metadata, enforce_min_max_values=True, enforce_rounding=True, locales=None
    ):
    ...
    def set_address_columns(self, column_names, anonymization_level="full"):
        ...
        if not isinstance(column_names, tuple):
            column_names = (
                tuple(column_names) if len(column_names) > 1 else (column_names,)
            )
    ...
    def get_parameters(self):
        ...
        for parameter_name in parameters:
            if parameter_name != "metadata":
                instantiated_parameters[parameter_name] = self.__dict__.get(
                    parameter_name
                )
    ...
    def _fit(self, processed_data, *args):
...
class BaseSingleTableSynthesizer(BaseSynthesizer):
        ...
        while num_valid < batch_size and counter < max_tries:
            ...
            if num_increase > 0:
                if output_file_path:
                    ...
                    append_kwargs = (
                        append_kwargs if os.path.getsize(output_file_path) > 0 else {}
                    )
    ...
    def _sample_with_progress_bar(
        ...
        try:
            with tqdm.tqdm(
                total=num_rows, disable=not show_progress_bar
            ) as progress_bar:
            ...
            handle_sampling_error(
                output_file_path == TMP_FILE_NAME, output_file_path, error
            )
    ...
    def sample(
        self, num_rows, max_tries_per_batch=100, batch_size=None, output_file_path=None
    ):
        ...
        try:
            with tqdm.tqdm(total=num_rows) as progress_bar:
                ...
                for condition_dataframe in conditions:
                    ...
                    sampled = pd.concat(
                        [sampled, sampled_for_condition], ignore_index=True
                    )

            is_reject_sampling = bool(
                hasattr(self, "_model")
                and not isinstance(self._model, GaussianMultivariate)
            )
            check_num_rows(
